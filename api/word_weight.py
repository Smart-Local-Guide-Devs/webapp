...
class WordWeight:
    ...
    def similar_apps_nltk(app_obj,data: DataFrame):
    	...
    	descriptions_df['descriptions_cleaned'] = descriptions_df.Descriptions.apply(lambda x: " ".join(re.sub(r'[^a-zA-Z]',' ',w).lower() for w in x.split() if re.sub(r'[^a-zA-Z]',' ',w).lower() not in stop_words_l) )
