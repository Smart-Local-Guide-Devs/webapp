...
class SimilarUserApps:
    ...
    def create_matrix(reviews: DataFrame, criteria: str) -> DataFrame:
        matrix = reviews.pivot_table(
            index="USER_NAME", columns="APP_ID", values=criteria
        )
    ...
    @staticmethod
    def recommend_item(
        user_name: str, reviews: DataFrame, criteria: str, items=3
    ) -> list:
        ...
        similar_users_df = DataFrame(similar_users, columns=["mean"])
